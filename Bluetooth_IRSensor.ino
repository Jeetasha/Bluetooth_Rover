...
//If 'R' command is recieved then the rover takes a right turn
  ...
  else if(t=='X')//Follow Line
  {
    if((digitalRead(Right_IR)==LOW)&&(digitalRead(Left_IR)==LOW))
    {
      move_Forward();
    }
  if((digitalRead(Right_IR) == 1)&&(digitalRead(Left_IR) == 0)){move_right();}
  if((digitalRead(Right_IR) == 0)&&(digitalRead(Left_IR) == 1)){move_left();}
  if((digitalRead(Right_IR) == 1)&&(digitalRead(Left_IR) == 1)){stop();}
  }
}
